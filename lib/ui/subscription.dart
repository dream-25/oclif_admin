...
class _SubscriptionScreenState extends State<SubscriptionScreen> {
  ...
  _getList() {
    ...
    Widget _enable = checkBox1a(
        context,
        strings.get(497),

        /// "Enable subscriptions for all providers",
        theme.mainColor,
        theme.style14W400,
    ...
    if (appSettings.enableSubscriptions) {
      ...
      list.add(documentBlock(
          strings.get(498) + ":" + strings.get(200), _controllerPromotion, "",
          () {
        setState(() {});
      }, strings.get(25)));

      /// "Promotion text",  "You can use HTML tags (<p>, <br>, <h1> and other)" // "Preview",
      list.add(SizedBox(
        height: 10,
      ));
      list.add(Row(
        children: [
          button2b(strings.get(9), () {
            /// "Save"
    ...
    if (_state == "edit") {
      ...
      Widget _default = checkBox1a(
        ...
        if (val == null) return;
        for (var item in subscriptions) item.defaultItem = false;
        currentSubscription.defaultItem = val;
        _redraw();
      });
      ...
      var _name =
          textElement2(strings.get(54), "", _controllerName, (String val) {
        /// "Name",
      ...
      var _text2 =
          textElement2(strings.get(492), "", _controllerText2, (String val) {
        /// "Text",
      ...
      Widget _price = Column(
        children: [
          ...
          /// If price is 0, it means that this period will be free, i.e. "Free trial"
        ],
      );

      Widget _days = numberElement2(
          strings.get(493),

          /// Days
          "31",
          "",
          _controllerDays, (String val) {
        /// pcs
        currentSubscription.days = toInt(val);
      });
      ...
      Widget _multiText = Row(
        ...
        children: [
          SelectableText(strings.get(494), style: aTheme.style14W400),
          ...
          Expanded(
              child: edit22(_controllerMultiText, "", theme.radius,
                  onChange: (String val) {
      ...
      Widget _color = Row(children: [
        ...
        Container(
          ...
          child: ElementSelectColor(
            getColor: () {
              return currentSubscription.color;
            },
      ...
      list.add(Row(
        children: [
          button2b(strings.get(495), () async {
          ...
          child: Column(
            children: [
              subscriptionItem(
                  210,
                  item,
                  item.price == 0 ? strings.get(503) : strings.get(504),
              ...
              Row(
                children: [
                  button2b(strings.get(62), () {
                    /// "Delete"
                    if (!appSettings.enableSubscriptions) return;
                    openDialog(dialogYesNoBody(
                      (String val) async {
                        if (val == "text2") {
                          if (subscriptions.length == 1)
                            return messageError(context, strings.get(500));

                          /// "You can't delete all subscriptions. For delete this, add new subscription before.",
                          waitInMainWindow(true);
                      ...
                      /// "Do you want to delete this item? You will can't recover this item."
                      strings.get(61),
  ...
  String? dataIntegrityCheck() {
    if (currentSubscription.text.isEmpty) return strings.get(91);

    /// "Please Enter Name",
    if (currentSubscription.text2.isEmpty) return strings.get(357);

    /// "Please Enter Text",
    // if (currentSubscription.price == 0)
    //   return strings.get(342); /// "Please Enter Price",
    if (currentSubscription.days == 0) return strings.get(496);
  ...
  _init() {
    if (currentSubscription.id.isNotEmpty) {
      _controllerName.text = getTextByLocale(
          currentSubscription.text, _mainModel.langEditDataComboValue);
      _controllerText2.text = getTextByLocale(
          currentSubscription.text2, _mainModel.langEditDataComboValue);
      _controllerPrice.text = currentSubscription.price.toString();
      _controllerDays.text = currentSubscription.days.toString();
      _controllerMultiText.text = getTextByLocale(
          currentSubscription.desc, _mainModel.langEditDataComboValue);
    } else {
