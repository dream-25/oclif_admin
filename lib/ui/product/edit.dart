...
class _EditInProductState extends State<EditInProduct> {
  ...
  _init() {
    ...
    if (currentArticle.id.isEmpty) {
      ...
      _controllerUnit.text = strings.get(447);

      /// pcs
      _controllerStock.text = "";
    } else {
      _controllerName.text = getTextByLocale(
          currentArticle.name, _mainModel.langEditDataComboValue);
      _controllerDescTitle.text = getTextByLocale(
          currentArticle.descTitle, _mainModel.langEditDataComboValue);
      _controllerDesc.text = getTextByLocale(
          currentArticle.desc, _mainModel.langEditDataComboValue);
      ...
      if (currentArticle.providers.isNotEmpty) {
        for (var item in _mainModel.provider.providersComboForProducts)
          if (item.id == currentArticle.providers[0])
            _mainModel.provider.providersComboValueForProduct =
                currentArticle.providers[0];
      }
    }
  }

  String? dataIntegrityCheck() {
    if (currentArticle.priceProduct == 0) return strings.get(472);

    /// "Price can't be 0",
    if (currentArticle.name.isEmpty) return strings.get(91);

    /// "Please Enter Name",
    if (currentArticle.priceProduct == 0) return strings.get(342);

    /// "Please enter price",
    if (currentArticle.gallery.isEmpty) return strings.get(428);
  ...
  Widget build(BuildContext context) {
    ...
    Widget _selectLanguage = Expanded(
        child: Container(
            width: 120,
            child: Combo(
              inRow: true,
    ...
    //
    if (isMobile()) {
      list.add(numberElement2Percentage(strings.get(130), "", _controllerTax,
          (String val) {
        /// "Tax",
      ...
      list.add(numberElement2Percentage(
          strings.get(266), "", _controllerTaxAdmin, (String val) {
      ...
      list.add(Row(
        children: [
          Expanded(
              child: numberElement2Percentage(
                  strings.get(130), "", _controllerTax, (String val) {
            /// "Tax",
            currentArticle.tax = toDouble(val);
          })),
          SizedBox(
      ...
      children: [
        ...
        Combo(
          ...
          onChange: (String value) {
            ...
            if (value != "root") currentArticle.providers.add(value);
    ...
    if (_expandDescription) {
      ...
      list.add(textElement2(strings.get(120), "", _controllerDescTitle,
          (String val) {
        /// Description Title
      ...
      list.add(documentBlock(
          strings.get(73) + ":" + strings.get(200),
          _controllerDesc,

          /// "Description:",  "You can use HTML tags (<p>, <br>, <h1> and other)"
          "", () {
    ...
    if (_expandVariants) {
      ...
      list.add(button2b(strings.get(434), () {
        /// "Add new group",
      ...
      for (var item in currentArticle.group) {
        ...
        for (var item2 in item.price) {
          if (item2.selected) {
            _nameVariant =
                getTextByLocale(item2.name, _mainModel.langEditDataComboValue);
          ...
          _variants.add(Column(
            children: [
              Text(
                item2.priceUnit + getPriceString(item2.price),
              ...
              Text(
                "${item2.stock} ${currentArticle.unit}",
                style: theme.style12W400,
              ),
              SizedBox(
                height: 5,
              ...
              variantButton(
                  _redraw, _mainModel.langEditDataComboValue, item, item2),
              SizedBox(
                height: 5,
              ),
              button2small(strings.get(68),
              ...
              children: [
                Row(
                  children: [
                    Text(
                      getTextByLocale(
                          item.name, _mainModel.langEditDataComboValue),
                      style: theme.style14W800,
                    ),
                    SizedBox(
                      width: 2,
    ...
    if (currentArticle.id.isEmpty)
      list.add(button2b(strings.get(425), () async {
        /// "Create new product",
        var ret = dataIntegrityCheck();
        if (ret != null) return messageError(context, ret);
        var _provider = currentArticle.providers.isNotEmpty
            ? getProviderById(currentArticle.providers[0])
            : null;
      ...
      list.add(Row(
        children: [
          Expanded(
              child: button2b(strings.get(427), () async {
          ...
          button2b(strings.get(426), () async {
            /// "Add New Product",
  ...
  _price() {
    Widget _price = numberElement2Price(
        strings.get(144),

        /// Price
        "123",
        "₹",
        _controllerPrice, (String val) {
      currentArticle.priceProduct = toDouble(val);
      _redraw();
    }, appSettings.digitsAfterComma);
    Widget _discPrice = numberElement2Price(
        strings.get(145),

        /// Discount price
        "123",
        "₹",
        _controllerDiscPrice, (String val) {
      currentArticle.discPriceProduct = toDouble(val);
      _redraw();
    }, appSettings.digitsAfterComma);
    Widget _unit = textElement2(
        strings.get(452),

        /// Unit
        strings.get(447),
        _controllerUnit, (String val) {
      /// pcs
      currentArticle.unit = val;
    });
    Widget _stock = numberElement2(
        strings.get(446),

        /// In Stock
        "9999",
        strings.get(447),
        _controllerStock, (String val) {
      /// pcs
      currentArticle.stock = toInt(val);
    });
    if (currentArticle.group.isNotEmpty) _stock = Container();
  ...
  Widget _priceSumma() {
    ...
    if (currentArticle.discPriceProduct != 0) {
      if (_selectedVariant && !_first)
        _priceVariants +=
            " + (${getPriceString(_plusMinus)}) = ${getPriceString(currentArticle.discPriceProduct + _plusMinus + _allPlusMinus)} ";
      _allPlusMinus = _plusMinus;
      _first = false;
      return Row(
        children: [
          Text(
            getPriceString(currentArticle.priceProduct),
            style: theme.style14W400U,
          ),
          SizedBox(
            width: 10,
          ),
          Text(
            getPriceString(currentArticle.discPriceProduct),
            style: theme.style14W400,
          ),
          // if (_plusMinus != 0)
          Text(
      ...
      if (_selectedVariant && !_first)
        _priceVariants +=
            " + (${getPriceString(_plusMinus)}) = ${getPriceString(currentArticle.priceProduct + _plusMinus + _allPlusMinus)} ";
      _allPlusMinus = _plusMinus;
      _first = false;
      return Text(
        getPriceString(currentArticle.priceProduct + _plusMinus),
        style: theme.style12W400,
      );
    }
  }
