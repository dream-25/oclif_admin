...
class _EditInOffersState extends State<EditInOffers> {
  ...
  Widget build(BuildContext context) {
    ...
    Widget _code =
        textElement2(strings.get(163), "", _controllerCode, (String val) {
      /// "CODE",
    ...
    Widget _desc =
        textElement2(strings.get(73), "", _controllerDesc, (String val) {
      /// Description
      currentOffer.setDesc(val, _mainModel.langEditDataComboValue);
      redrawMainWindow();
    });

    Widget _discount = numberElement2Percentage(
        strings.get(164),
    ...
    Widget _discountType = Row(
      children: [
        ...
        Expanded(
          ...
          onChange: (String value) {
            if (value == "percentage") if (toInt(_controllerDiscount.text) >
                100) _controllerDiscount.text = "100";
    ...
    Widget _visibleInApp = Column(
      children: [
        checkBox1a(
            ...
            /// "Visible in app. (If uncheck: in app offer don't showing. Customer can only enter text code)",
            theme.mainColor,
            theme.style14W400,
        ...
        textElement2(strings.get(441), "", _controllerUrl, (String val) {
        ...
        Row(children: [
          ...
          Container(
            width: 150,
            child: ElementSelectColor(
              getColor: () {
                return currentOffer.color;
              },
              setColor: (Color val) {
    ...
    if (DateTime.now().isAfter(currentOffer.expired)) {
      _inAppPreview = Center(
          child: Text(
        strings.get(167),
        style: TextStyle(fontSize: 40, color: Colors.red.withAlpha(100)),
      ));
      ...
      var disc = "";
      var desc =
          getTextByLocale(currentOffer.desc, _mainModel.langEditDataComboValue);
      if (currentOffer.discountType == "fixed")
      ...
      _inAppPreview = currentOffer.visibleForUser
          ? offerWidget(
              ...
              /// "Use now"
              "${strings.get(509)} ${appSettings.getDateTimeString(currentOffer.expired)}",
              theme.style12W800)
    ...
    list.add(Row(
      children: [
        ...
        Expanded(
            child: popupWidget(_popupKey, _providersComboData, (String value) {
          _providersComboData.value = value;
          _serviceComboData = getComboData("service", strings.get(254),

              /// All
              providersForServices: _providersComboData);
          _redraw();
        }, strings.get(60), selectMany: true))
    ...
    list.add(Row(
      children: [
        ...
        Expanded(
            child: popupWidget(_popupKey2, _categoryComboData, (String value) {
    ...
    list.add(Row(
      children: [
        ...
        Expanded(
            child: popupWidget(_popupKey3, _serviceComboData, (String value) {
    ...
    list.add(Row(
      children: [
        ...
        Expanded(
            child: popupWidget(_popupKey4, _articleComboData, (String value) {
    ...
    if (currentOffer.id.isEmpty)
      list.add(button2b(strings.get(169), () async {
        /// "Create new offer",
      ...
      list.add(Row(
        children: [
          Expanded(
              child: button2b(strings.get(170), () async {
            /// "Save current offer",
            var ret = dataIntegrityCheck();
            if (ret != null) return messageError(context, ret);
            ret = await offerSave();
  ...
  String? dataIntegrityCheck() {
    if (currentOffer.code.isEmpty) return strings.get(341);

    /// Please enter code
    if (currentOffer.discount == 0) return strings.get(507);

    /// Please enter discount

    currentOffer.providers = [];
    for (var item in _providersComboData.data)
      if (item.checkSelected) currentOffer.providers.add(item.id);

    currentOffer.services = [];
    for (var item in _serviceComboData.data)
      if (item.checkSelected) currentOffer.services.add(item.id);

    currentOffer.category = [];
    for (var item in _categoryComboData.data)
      if (item.checkSelected) currentOffer.category.add(item.id);

    currentOffer.article = [];
    for (var item in _articleComboData.data)
      if (item.checkSelected) currentOffer.article.add(item.id);
  ...
  _init() {
    _providersComboData = getComboData("providers", strings.get(254));

    /// All
    _categoryComboData = getComboData("category", strings.get(254));

    /// All
    _serviceComboData = getComboData("service", strings.get(254));

    /// All
    _articleComboData = getComboData(
      "article",
      strings.get(254),
    );
    ...
    if (currentOffer.id.isNotEmpty) {
      ...
      for (var item in currentOffer.providers)
        for (var item2 in _providersComboData.data)
          if (item2.id == item) item2.checkSelected = true;
