...
class _Dashboard2ScreenState extends State<Dashboard2Screen>
  ...
  _initMenu() {
    ...
    menu1.add(
      Menu4Data("products", strings.get(423),
  ...
  Widget build(BuildContext context) {
    ...
    if (user == null) {
      ...
      drawState((String val) {
        state = val;
        _redraw();
      }, context, _redraw, strings.locale, strings.direction);
      return Scaffold(
    ...
    if (state == "blog") _child = BlogScreen();
    if (state == "banners") _child = BannersScreen();
    if (state == "favorite") _child = FavoritesScreen();
    if (state == "payout") _child = PayoutsScreen();
    if (state == "earning") _child = EarningScreen();
    if (state == "booking") _child = BookingScreen();
    if (state == "customers") _child = CustomersScreen();
    if (state == "review") _child = ReviewScreen();
    if (state == "offers") _child = OffersScreen();
    if (state == "chat") _child = ChatScreen();
    if (state == "home") _child = HomeScreen();
    // users
    // if (_state == "roles")
    //   _child = RolesScreen();
    // if (_state == "users")
    //   _child = UsersScreen();
    // providers
    if (state == "provider") _child = ProvidersScreen();
    ...
    // services
    if (state == "service") _child = ServicesScreen();
    if (state == "products") _child = ArticlesScreen();
    // settings
    if (state == "general") _child = SettingsScreen();
    if (state == "currency") _child = CurrencyScreen();
    if (state == "share") _child = ShareScreen();
    if (state == "gateways") _child = GatewaysScreen();
    if (state == "otp") _child = OTPScreen();
    if (state == "documents") _child = DocumentsScreen();
    if (state == "booking_status") _child = BookingStatusesScreen();
    if (state == "sample_data") _child = SampleDataScreen();
    if (state == "profile") _child = ProfileScreen();

    // apps settings
    if (state == "appSettings") _child = AppServiceSettingsScreen();
    if (state == "providerAppSettings") _child = ProviderAppSettingsScreen();
    if (state == "webSiteAppSettings") _child = WebSiteAppSettingsScreen();
    if (state == "adminPanelAppSettings")
      _child = AdminPanelAppSettingsScreen();
    if (state == "appslang") _child = AppLangScreen();
    if (state == "notify") _child = NotifyScreen();
    if (state == "category") _child = CategoryScreen();
    if (state == "mspos") _child = ElementPositionScreen();
    if (state == "listService") _child = ListServiceScreen();
    if (state == "repair") _child = RepairScreen();
    if (state == "subscriptions") _child = SubscriptionScreen();

    drawState(
        (String _) {}, context, _redraw, strings.locale, strings.direction);
                  ...
                  children: <Widget>[
                    Container(
                        ...
                        child: ScrollConfiguration(
                            behavior: MyCustomScrollBehavior(), child: _child)),
                      ...
                      appbar36(
                          ...
                          Container(
                            ...
                            child: Row(
                              children: [
                                Text(strings.get(12),
                                    style: theme.style14W800), // "Dark Mode",
                                SizedBox(
                                  width: 10,
                                ),
                                Container(
                                    ...
                                    child: CheckBox48(
                                      onChanged: (bool value) async {
                                        ...
                                        _redraw();
                                        var ret = await saveAdminPanelDarkMode(
                                            theme.darkMode);
                                        if (ret != null)
