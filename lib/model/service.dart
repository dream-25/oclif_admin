...
class MainDataService {
  ...
  changeProvider() {
    ...
    for (var item in providers)
      if (item.select) currentProduct.providers.add(item.id);
  }
  ...
  Future<String?> load(BuildContext context) async {
    initService("all", "", () {
      ...
      if (parent.serviceSortFilter != null) parent.serviceSortFilter!(null);
      redrawMainWindow();
    });
    return null;
  }
  ...
  _setCategoryAndProvider() {
    for (var item in categories) {
      ...
      for (var item2 in currentProduct.category)
        if (item2 == item.id) item.select = true;
    }
    for (var item in providers) {
      item.select = false;
  ...
  PriceData getPrice() {
    ...
    for (var item in currentProduct.price) {
      ...
      } else if (item.price < _price) {
        _price = item.price;
        currentPrice = item;
      }
    }
    if (_price == double.maxFinite) _price = 0;
    return currentPrice;
  }
  ...
  String csv() {
    ...
    for (var item in product) {
      t2.add([
        item.id,
        parent.getTextByLocale(item.name),
        parent.getTextByLocale(item.desc),
        getPriceString(item.tax),
  ...
  String getServiceMinPrice(ProductData item) {
    ...
    for (var item in item.price) {
      ...
      } else if (item.price < _price) _price = item.price;
    }
    if (_price == double.maxFinite) _price = 0;
  ...
  getPriceString(double price) {
    if (appSettings.rightSymbol) {
      ...
      return "${"₹"}${price.toStringAsFixed(appSettings.digitsAfterComma)}";
    }
    return "${price.toStringAsFixed(appSettings.digitsAfterComma)}${"₹"}";
  }
  ...
  Future<String?> saveInMainScreenServices() async {
    ...
    for (var item in product)
      if (item.select) appSettings.inMainScreenServices.add(item.id);
    //
    try {
      await FirebaseFirestore.instance.collection("settings").doc("main").set(
          {"inMainScreenServices": appSettings.inMainScreenServices},
  ...
  List<AddonData>? getProviderAddons() {
    if (currentProduct.providers.isNotEmpty)
      for (var item in providers)
        if (item.id == currentProduct.providers[0]) return item.addon;
    return null;
  }
}

ProductData currentTestService = ProductData(
  "1",
  ...
  visible: true,
  price: [
    PriceData([StringData(code: "en", text: "Carpet shampooing Hi")], 20, 10,
        "fixed", ImageData())
  ],
  gallery: [],
